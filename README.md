...
- **`src/seo.ts`** builds the JSON-LD graph (the studio, its two packages and
  their prices, the website node). Prices come from `src/pricing.ts`, the same
  module the pricing section renders from.
- **`src/pricing.ts`** is the one place the landing page and retainer prices
  are computed. Set `PRICING_BASE` to override the landing page starting price
  (defaults to `1199`); the retainer is always `PRICING_BASE + 2300`.
